...
describe("auditMemory — orphan references", () => {
  test("detects orphan association reference", () => {
    ...
    const result = core.auditMemory(frags);
...
describe("auditMemory — duplicate IDs", () => {
  test("detects duplicate IDs across fragments", () => {
    ...
    const result = core.auditMemory([f1, f2]);
    assert.equal(result.healthy, false);
...
describe("auditMemory — confidence anomalies", () => {
  test("detects confidence above 1.0", () => {
    const f = core.createFragment("test", "ai");
    f.confidence = 1.5;
    const result = core.auditMemory([f]);
    assert.equal(result.healthy, false);
...
describe("auditMemory — missing required fields", () => {
  test("detects missing fragment text", () => {
    const f = core.createFragment("temp", "ai");
    delete f.fragment;
    const result = core.auditMemory([f]);
    assert.equal(result.healthy, false);
...
describe("formatAuditReport", () => {
  test("formats clean report", () => {
    const result = { total_fragments: 5, issues_found: 0, issues: [], healthy: true };
    const output = core.formatAuditReport(result);
  ...
  test("formats empty memory report", () => {
    const result = { total_fragments: 0, issues_found: 0, issues: [], healthy: true };
    const output = core.formatAuditReport(result);
