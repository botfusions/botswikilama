...
describe("updateGuide", () => {
  ...
  test("multiple updates accumulate (anti_patterns don't overwrite)", () => {
    ...
    const updated = guides.updateGuide(gs, "react", { add_anti_patterns: ["second"] });
  ...
  test("multiple updates accumulate (pitfalls don't overwrite)", () => {
    ...
    const updated = guides.updateGuide(gs, "react", { add_pitfalls: ["pitfall-b"] });
  ...
  test("deprecated=false does not change already deprecated guide", () => {
    ...
    const updated = guides.updateGuide(gs, "react", { deprecated: false });
    assert.equal(updated.deprecated, true);
  });

  test("finds guide case-insensitively for update", () => {
    const gs = [seedGuide()];
    const updated = guides.updateGuide(gs, "REACT", { description: "found case-insensitive" });
    assert.ok(updated);
