...
**Vulnerability:** Array arguments (technologies, lessons, etc.) lacked validation for the length of individual string items. An attacker could provide a small array containing extremely large strings, bypassing array count limits and causing resource exhaustion during processing.
**Learning:** Validating array count is insufficient for DoS protection when the items themselves are variable-length strings. Defense in depth requires validating both the number of items and the size of each item.
**Prevention:** Use a centralized `validateArrayItems` helper to enforce maximum string lengths for all elements within user-provided arrays in tool handlers.
