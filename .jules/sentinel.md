...
## 2026-05-14 - YAML Injection in Wiki Frontmatter
**Vulnerability:** User-provided values (titles, project names, etc.) were directly inserted into Markdown frontmatter without sanitization. This allowed injecting additional YAML keys or altering the structure of the frontmatter.
**Learning:** Any user-controlled input used in generating structured data formats like YAML must be properly escaped and quoted. In Markdown frontmatter, wrapping values in double quotes and escaping inner double quotes, backslashes, and newlines prevents most injection attacks.
**Prevention:** Implement a `sanitizeYamlValue` helper that handles escaping and quoting, and apply it to all user-influenced variables in frontmatter templates.
