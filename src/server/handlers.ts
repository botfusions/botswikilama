...
export async function handleSessionStart(args?: SessionStartArgs): Promise<ToolResult> {
  const v = validateLengths(args, { task_type: MAX_LENGTHS.task_type, initial_approach: MAX_LENGTHS.approach });
  if (v) return v;
...
export async function handleSessionEnd(args?: SessionEndArgs): Promise<ToolResult> {
  const v = validateLengths(args, { outcome: MAX_LENGTHS.outcome, final_approach: MAX_LENGTHS.approach });
  if (v) return v;
...
export async function handleGuideMerge(args?: GuideMergeArgs): Promise<ToolResult> {
  const c = validateCounts(args, { guides: MAX_COUNTS.guides, contexts: MAX_COUNTS.contexts, learnings: MAX_COUNTS.learnings });
  if (c) return c;
...
export async function handleWikiIngest(args?: WikiIngestArgs): Promise<ToolResult> {
  ...
  if (v) return v;
  const c = validateCounts(args, { entities: MAX_COUNTS.entities, concepts: MAX_COUNTS.concepts, decisions: MAX_COUNTS.decisions });
  if (c) return c;
