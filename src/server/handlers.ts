...
export async function handleWikiSetup(args?: WikiSetupArgs): Promise<ToolResult> {
  const vaultPathRaw = args?.vault_path;
  const projectName = args?.project_name || path.basename(vaultPathRaw || "wiki");
  ...
  try {
    const vaultPath = wiki.validateVaultPath(vaultPathRaw);
