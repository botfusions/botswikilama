...
export async function handleGuidePractice(args?: GuidePracticeArgs): Promise<ToolResult> {
  ...
  if (c) return c;
  const i = validateArrayItems(args, { contexts: MAX_LENGTHS.name, learnings: MAX_LENGTHS.name });
  if (i) return i;
...
export async function handleGuideUpdate(args?: GuideUpdateArgs): Promise<ToolResult> {
  ...
  if (c) return c;
  const i = validateArrayItems(args, { add_anti_patterns: MAX_LENGTHS.name, add_pitfalls: MAX_LENGTHS.name });
  if (i) return i;
...
export async function handleGuideMerge(args?: GuideMergeArgs): Promise<ToolResult> {
  ...
  if (c) return c;
  const i = validateArrayItems(args, { guides: MAX_LENGTHS.name, contexts: MAX_LENGTHS.name, learnings: MAX_LENGTHS.name });
  if (i) return i;
...
export async function handleWikiIngest(args?: WikiIngestArgs): Promise<ToolResult> {
  ...
  if (c) return c;
  const i = validateArrayItems(args, { entities: MAX_LENGTHS.name, concepts: MAX_LENGTHS.name, decisions: MAX_LENGTHS.name });
  if (i) return i;
