...
export async function handleWikiIngest(args?: WikiIngestArgs): Promise<ToolResult> {
  ...
  try {
    ...
    const safeSource = wiki.sanitizeYamlValue(filePath || "manual");
    ...
    for (const entity of entities) {
      ...
      if (!fs.existsSync(entityPath)) {
        const safeEntity = wiki.sanitizeYamlValue(entity);
        const entityContent = `---\ntitle: ${safeEntity}\ntags: [entity]\ndate: ${date}\nstatus: active\n---\n\n# ${entity}\n\n## Sources\n\n- [[${date}-${slug}]]\n\n## Related\n`;
    ...
    for (const concept of concepts) {
      ...
      if (!fs.existsSync(conceptPath)) {
        const safeConcept = wiki.sanitizeYamlValue(concept);
    ...
    for (const decision of decisions) {
      ...
      const safeDecision = wiki.sanitizeYamlValue(decision);
