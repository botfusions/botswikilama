...
export async function handleWikiIngest(args?: WikiIngestArgs): Promise<ToolResult> {
  ...
  try {
    ...
    let pageContent = `---\ntitle: ${wiki.sanitizeYamlValue(title || "Untitled")}\ntags: [source]\nsource: ${wiki.sanitizeYamlValue(filePath || "manual")}\ndate: ${date}\nstatus: active\n---\n\n# ${title || "Untitled"}\n\n${summary}\n\n## Sources\n\n${filePath ? `- ${path.basename(filePath)}` : "- Manual entry"}\n\n## Related\n`;
    ...
    for (const entity of entities) {
      ...
      if (!fs.existsSync(entityPath)) {
        const entityContent = `---\ntitle: ${wiki.sanitizeYamlValue(entity)}\ntags: [entity]\ndate: ${date}\nstatus: active\n---\n\n# ${entity}\n\n## Sources\n\n- [[${date}-${slug}]]\n\n## Related\n`;
    ...
    for (const concept of concepts) {
      ...
      if (!fs.existsSync(conceptPath)) {
        const conceptContent = `---\ntitle: ${wiki.sanitizeYamlValue(concept)}\ntags: [concept]\ndate: ${date}\nstatus: active\n---\n\n# ${concept}\n\n## Sources\n\n- [[${date}-${slug}]]\n\n## Related\n`;
    ...
    for (const decision of decisions) {
      ...
      const decisionContent = `---\ntitle: ${wiki.sanitizeYamlValue(decision)}\ntags: [decision]\ndate: ${date}\nstatus: active\n---\n\n# ${decision}\n\n## Sources\n\n- [[${date}-${slug}]]\n\n## Related\n`;
