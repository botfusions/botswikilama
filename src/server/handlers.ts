...
export async function handleWikiSetup(args?: WikiSetupArgs): Promise<ToolResult> {
  ...
  let projectName = args?.project_name || path.basename(vaultPath || "wiki");

  if (projectName.includes("/") || projectName.includes("\\") || projectName.includes("..")) {
    return { content: [{ type: "text", text: "Error: 'project_name' contains invalid characters" }], isError: true };
  }
