...
export async function handleWikiSetup(args?: WikiSetupArgs): Promise<ToolResult> {
  const vaultPathArg = args?.vault_path;
  const projectName = args?.project_name || path.basename(vaultPathArg || "wiki");
  ...
  try {
    const vaultPath = wiki.validateVaultPath(vaultPathArg);
...
export async function handleWikiIngest(args?: WikiIngestArgs): Promise<ToolResult> {
  ...
  try {
    ...
    if (filePath) {
      ...
      if (!resolvedFile.startsWith(vaultPath + path.sep)) {
        return { content: [{ type: "text", text: `Error: file_path must be within the vault_path` }], isError: true };
      }
    }
