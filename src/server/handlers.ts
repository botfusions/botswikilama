...
export async function handleWikiIngest(args?: WikiIngestArgs): Promise<ToolResult> {
  ...
  try {
    ...
    const sanitizedTitle = wiki.sanitizeYamlValue(title || "Untitled");
    const sanitizedFilePath = wiki.sanitizeYamlValue(filePath || "manual");

    let pageContent = `---\ntitle: ${sanitizedTitle}\ntags: [source]\nsource: ${sanitizedFilePath}\ndate: ${date}\nstatus: active\n---\n\n# ${title || "Untitled"}\n\n${summary}\n\n## Sources\n\n${filePath ? `- ${path.basename(filePath)}` : "- Manual entry"}\n\n## Related\n`;
    ...
    for (const entity of entities) {
      ...
      if (!fs.existsSync(entityPath)) {
        const sanitizedEntity = wiki.sanitizeYamlValue(entity);
        const entityContent = `---\ntitle: ${sanitizedEntity}\ntags: [entity]\ndate: ${date}\nstatus: active\n---\n\n# ${entity}\n\n## Sources\n\n- [[${date}-${slug}]]\n\n## Related\n`;
    ...
    for (const concept of concepts) {
      ...
      if (!fs.existsSync(conceptPath)) {
        const sanitizedConcept = wiki.sanitizeYamlValue(concept);
        const conceptContent = `---\ntitle: ${sanitizedConcept}\ntags: [concept]\ndate: ${date}\nstatus: active\n---\n\n# ${concept}\n\n## Sources\n\n- [[${date}-${slug}]]\n\n## Related\n`;
    ...
    for (const decision of decisions) {
      ...
      const sanitizedDecision = wiki.sanitizeYamlValue(decision);
      const decisionContent = `---\ntitle: ${sanitizedDecision}\ntags: [decision]\ndate: ${date}\nstatus: active\n---\n\n# ${decision}\n\n## Sources\n\n- [[${date}-${slug}]]\n\n## Related\n`;
