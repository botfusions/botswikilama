...
export async function handleGuideUpdate(args?: GuideUpdateArgs): Promise<ToolResult> {
  ...
  if (c) return c;
  const ai = validateArrayItems(args, { add_anti_patterns: MAX_LENGTHS.description, add_pitfalls: MAX_LENGTHS.description });
  if (ai) return ai;
...
export async function handleGuideMerge(args?: GuideMergeArgs): Promise<ToolResult> {
  ...
  if (c) return c;
  const ai = validateArrayItems(args, { guides: MAX_LENGTHS.name, contexts: MAX_LENGTHS.name, learnings: MAX_LENGTHS.description });
  if (ai) return ai;
