...
export async function handleSessionStart(args?: SessionStartArgs): Promise<ToolResult> {
  ...
  if (c) return c;
  const a = validateArrayItems(args, { technologies: MAX_LENGTHS.name });
  if (a) return a;
...
export async function handleSessionEnd(args?: SessionEndArgs): Promise<ToolResult> {
  ...
  if (c) return c;
  const a = validateArrayItems(args, { lessons: MAX_LENGTHS.description });
  if (a) return a;
...
export async function handleMemoryRead(args?: MemoryReadArgs): Promise<ToolResult> {
  ...
  if (c) return c;
  const a = validateArrayItems(args, { ids: MAX_LENGTHS.id });
  if (a) return a;
...
export async function handleGuidePractice(args?: GuidePracticeArgs): Promise<ToolResult> {
  ...
  if (c) return c;
  const a = validateArrayItems(args, { contexts: MAX_LENGTHS.name, learnings: MAX_LENGTHS.description });
  if (a) return a;
...
export async function handleGuideUpdate(args?: GuideUpdateArgs): Promise<ToolResult> {
  ...
  if (c) return c;
  const a = validateArrayItems(args, { add_anti_patterns: MAX_LENGTHS.description, add_pitfalls: MAX_LENGTHS.description });
  if (a) return a;
...
export async function handleGuideMerge(args?: GuideMergeArgs): Promise<ToolResult> {
  ...
  if (c) return c;
  const a = validateArrayItems(args, { guides: MAX_LENGTHS.name, contexts: MAX_LENGTHS.name, learnings: MAX_LENGTHS.description });
  if (a) return a;
...
export async function handleWikiIngest(args?: WikiIngestArgs): Promise<ToolResult> {
  ...
  try {
    ...
    const a = validateArrayItems(args, { entities: MAX_LENGTHS.name, concepts: MAX_LENGTHS.name, decisions: MAX_LENGTHS.name });
    if (a) return a;
