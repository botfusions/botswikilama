...
export async function handleGuidePractice(args?: GuidePracticeArgs): Promise<ToolResult> {
  ...
  if (c) return c;
  const aiContexts = validateArrayItems(args, "contexts", MAX_LENGTHS.name);
  if (aiContexts) return aiContexts;
  const aiLearnings = validateArrayItems(args, "learnings", MAX_LENGTHS.description);
  if (aiLearnings) return aiLearnings;
...
export async function handleGuideUpdate(args?: GuideUpdateArgs): Promise<ToolResult> {
  ...
  if (c) return c;
  const aiAntiPatterns = validateArrayItems(args, "add_anti_patterns", MAX_LENGTHS.description);
  if (aiAntiPatterns) return aiAntiPatterns;
  const aiPitfalls = validateArrayItems(args, "add_pitfalls", MAX_LENGTHS.description);
  if (aiPitfalls) return aiPitfalls;
...
export async function handleGuideMerge(args?: GuideMergeArgs): Promise<ToolResult> {
  ...
  if (c) return c;
  const aiGuides = validateArrayItems(args, "guides", MAX_LENGTHS.name);
  if (aiGuides) return aiGuides;
  const aiContexts = validateArrayItems(args, "contexts", MAX_LENGTHS.name);
  if (aiContexts) return aiContexts;
  const aiLearnings = validateArrayItems(args, "learnings", MAX_LENGTHS.description);
  if (aiLearnings) return aiLearnings;
...
export async function handleSessionStats(args?: SessionStatsArgs): Promise<ToolResult> {
  if (args?.count !== undefined) {
    if (typeof args.count !== "number" || Number.isNaN(args.count) || args.count < 1 || args.count > 100) {
      return {
        content: [{ type: "text", text: "Error: 'count' must be a number between 1 and 100" }],
        isError: true,
      };
    }
  }
...
export async function handleWikiIngest(args?: WikiIngestArgs): Promise<ToolResult> {
  ...
  const aiEntities = validateArrayItems(args, "entities", MAX_LENGTHS.name);
  if (aiEntities) return aiEntities;
  const aiConcepts = validateArrayItems(args, "concepts", MAX_LENGTHS.name);
  if (aiConcepts) return aiConcepts;
  const aiDecisions = validateArrayItems(args, "decisions", MAX_LENGTHS.name);
  if (aiDecisions) return aiDecisions;
