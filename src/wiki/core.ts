...
export function validateVaultPath(vaultPath: string): string {
  ...
  // To prevent symlink traversal attacks, resolve realpath for existing parts of the path
  ...
  while (currentPath && currentPath !== path.parse(currentPath).root) {
    try {
      if (fs.existsSync(currentPath)) {
        ...
        absolutePath = suffix ? path.join(real, suffix) : real;
        break;
      }
    } catch {}
    suffix = suffix ? path.join(path.basename(currentPath), suffix) : path.basename(currentPath);
...
/**
 * Sanitizes a string for use as a YAML value in frontmatter.
 * It wraps the value in double quotes and escapes special characters.
 */
...
function generateIndexTemplate(projectName: string, language: string): string {
  ...
  const title = sanitizeYamlValue(`${projectName} — İçerik Kataloğu`);
  return `---
title: ${title}
date: ${date}
---
